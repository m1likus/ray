...
bool IntersectTriangle (SRay ray, vec3 v1, vec3 v2, vec3 v3, out float time )
{
	...
	if (abs(NdotRayDirection) < 0.001) return false; /* they are parallel so they don't intersect */
	...
	vec3 edge1 = v2 - v1; /*edge 0 */
	vec3 VP1 = P - v1;
	C = cross(edge1, VP1);
	if (dot(N, C) < 0) return false; /* P is on the right side	*/
	vec3 edge2 = v3 - v2; /* edge 1*/
	vec3 VP2 = P - v2;
	C = cross(edge2, VP2);
	if (dot(N, C) < 0) return false; /* P is on the right side*/
	vec3 edge3 = v1 - v3; /* edge 2*/
	vec3 VP3 = P - v3;
	C = cross(edge3, VP3);
	if (dot(N, C) < 0) return false; /* P is on the right side */
	time = t;
...
bool Raytrace(SRay ray, SSphere spheres[2], STriangle triangles[12], SMaterial materials[2], float start, float final, inout SIntersection intersect)
{
	bool result = false;
	float test = start;
...
void initializeDefaultLightMaterials(out SLight light, out SMaterial materials[2])
{
	...
	materials[0].Color = vec3(0.4, 0.0, 0.0);
...
void main(void)
{
	...
	uCamera = SCamera(vec3(0.0, 0.0, -8.0), vec3(0.0, 0.0, 1.0), vec3(0.0, 1.0, 0.0), vec3(1.0, 0.0, 0.0), vec2(1.0));
